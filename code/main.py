...
if __name__ == "__main__":
    ...
    for file in tqdm(mp4_files):    #file: video file path
        ...
        srt_extraction.main(video_path, srt_path)       #提取字幕
        with open(srt_path, 'r', encoding='utf-8') as file:
            srt_number = len(file.readlines())//4 
        srt_translation.main(srt_path, srt_translated_path, chatgpt_api, prompt_path, srt_number)       #翻译字幕
        video_subtitle_merging.main(video_name, video_path, srt_translated_path, video_subtitled_folder_path) #字幕与视频合并
